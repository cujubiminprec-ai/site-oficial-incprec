...
router.post(
  ...
  [
    ...
    body("descricao_pedido").notEmpty().isLength({ max: 500 }).withMessage("Descrição obrigatória (máx. 500)"),
  ],
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { nome, email, cpf, telefone, descricao_pedido } = req.body as {
    ...
    try {
      await query(
        ...
        [
          ...
          descricao_pedido, prazoLegal, req.ip ?? null, criadoEm, criadoEm,
        ]
      );
    ...
    try {
      ...
      await query(
        `UPDATE lai_pedidos
         ...
         WHERE id = ?`,
        [resposta, status ?? "respondido", agora, req.user!.userId, agora, req.params.id]
      );
...
router.patch("/:id/status", autenticar, exigirPermissao("lai-admin"), async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    await query(
      "UPDATE lai_pedidos SET status = ?, atualizado_em = ? WHERE id = ?",
