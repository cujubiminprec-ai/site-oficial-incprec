...
router.post(
  ...
  [
    ...
    body("mensagem").notEmpty().isLength({ max: 500 }).withMessage("Mensagem obrigatória (máx. 500 chars)"),
  ],
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { tipo, assunto, mensagem, nome, email, cpf, telefone, anonimo } = req.body as {
      tipo: string; assunto: string; mensagem: string; nome?: string; email?: string;
    ...
    try {
      ...
      await query(
        `UPDATE ouvidoria_mensagens
         SET resposta = ?, status = ?, respondido_em = ?, respondido_por = ?, atualizado_em = ?
         WHERE id = ?`,
        [resposta, status ?? "respondida", agora, req.user!.userId, agora, req.params.id]
      );
...
router.patch("/:id/status", autenticar, exigirPermissao("ouvidoria-admin"), async (_req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    await query(
      "UPDATE ouvidoria_mensagens SET status = ?, atualizado_em = ? WHERE id = ?",
      [status, new Date().toISOString(), _req.params.id]
    );
