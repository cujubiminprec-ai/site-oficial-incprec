...
import { uploadUnico, uploadMultiplo, urlPublica, PastaUpload } from "../middleware/upload";
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const pasta     = (req.query.pasta as PastaUpload) ?? "documentos";
    ...
    try {
      ...
      res.status(500).json({ sucesso: false, mensagem: "Arquivo enviado, mas não foi possível registrar no banco SQLite." });
    }
  }
);
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const pasta     = (req.query.pasta as PastaUpload) ?? "documentos";
    const resultados: { id: number; nome: string; url: string; tipo_mime: string; tamanho: number; pasta: string }[] = [];
...
router.get("/", autenticar, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const result = await query(
      `SELECT id, nome_original, pasta, url_publica, tipo_mime, tamanho_bytes, criado_em
