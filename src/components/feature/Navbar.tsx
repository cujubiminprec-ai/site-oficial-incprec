...
import { loadMenuItems, MenuItem, defaultMenuItems } from "@/pages/admin/tabs/MenuNavegacaoTab";

function useContrachequeUrl() {
  return localStorage.getItem("inprec_contracheque_url") || "";
}

function useRedes() {
  return {
    facebook: localStorage.getItem("inprec_facebook") || "https://facebook.com/inprec",
    instagram: localStorage.getItem("inprec_instagram") || "https://instagram.com/inprec",
    youtube: localStorage.getItem("inprec_youtube") || "https://youtube.com/@inprec",
    linkedin: localStorage.getItem("inprec_linkedin") || "",
  };
}
...
export default function Navbar() {
  ...
  const [navItems, setNavItems] = useState<MenuItem[]>(() => {
    const loaded = loadMenuItems().filter((m) => m.ativo);
    return loaded.length > 0 ? loaded : defaultMenuItems.filter((m) => m.ativo);
  });
  const location = useLocation();
  const { config } = useSiteConfig();
  const contrachequeUrl = useContrachequeUrl();
  ...
  useEffect(() => {
    const loaded = loadMenuItems().filter((m) => m.ativo);
    setNavItems(loaded.length > 0 ? loaded : defaultMenuItems.filter((m) => m.ativo));
  }, [location]);
  ...
  return (
    ...
    >
      ...
      <nav className="w-full px-4 md:px-6 flex items-center justify-between h-[72px] md:h-[84px]">
        ...
        {/* Selo Pró-Gestão */}
        {config.proGestaoVisivel && config.proGestaoSeloUrl && (
          <Link
            to={config.proGestaoLink || "/pro-gestao"}
            className="hidden md:flex items-center ml-2 cursor-pointer flex-shrink-0 group"
            title="Pró-Gestão RPPS"
          >
            <img
              src={config.proGestaoSeloUrl}
              alt="Selo Pró-Gestão RPPS"
              className="h-10 md:h-14 w-auto object-contain transition-transform duration-200 group-hover:scale-105 drop-shadow-md"
            />
          </Link>
        )}
