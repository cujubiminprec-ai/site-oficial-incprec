...
function TextoBloco({ bloco, primaryColor }: { bloco: BlocoConteudo; primaryColor: string }) {
  ...
  // Obter link dinâmico do menu PRO-GESTÃO
  const getProGestaoLink = () => {
    try {
      const stored = localStorage.getItem("inprec_transparencia_menus");
      if (stored) {
        const parsed = JSON.parse(stored);
        const pro = parsed.find((m: any) =>
          m.titulo.toUpperCase().includes("PRO-GESTÃO") || m.titulo.toUpperCase().includes("PRO GESTÃO")
        );
        if (pro && pro.href) return pro.href;
      }
    } catch (e) {
      console.error(e);
    }
    return "https://transparencia.cujubim.ro.gov.br/novo/PREVIDENCIA/publicacoes/documentos/2026/10?grupo=141&perPage=10&page=1";
  };

  const proGestaoLink = config.proGestaoLink || getProGestaoLink();
  const isExternal = proGestaoLink.startsWith("http");
    ...
    return (
      <section className="py-12 px-4 animate-fade-in">
        <div className="max-w-screen-xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 bg-white p-6 md:p-8 rounded-2xl border border-gray-100 hover:border-gray-200 transition-all duration-300">
          ...
          {/* Selo Pró-Gestão interativo */}
          {config.proGestaoSeloUrl ? (
            <a
              href={proGestaoLink}
              target={isExternal ? "_blank" : undefined}
              rel={isExternal ? "noopener nofollow noreferrer" : undefined}
              className="flex-shrink-0 flex flex-col items-center justify-center p-6 bg-gradient-to-br from-amber-50/50 to-white rounded-2xl border border-amber-100 hover:border-amber-200 hover:shadow-lg transition-all duration-300 cursor-pointer group text-center w-full md:w-72"
            >
              <div className="relative mb-4">
                <div className="absolute inset-0 rounded-full bg-amber-400/10 animate-pulse scale-110"></div>
                <img
                  src={config.proGestaoSeloUrl}
                  alt="Selo Pró-Gestão RPPS"
                  className="h-28 md:h-32 w-auto object-contain transition-transform duration-300 group-hover:scale-105 drop-shadow-md relative z-10"
                />
              </div>
              <h4 className="text-xs font-bold text-gray-800 uppercase tracking-wider mb-1">Certificação Pró-Gestão</h4>
              <p className="text-[10px] text-gray-500 leading-snug max-w-[200px] mb-3">Acesse a pasta de publicações e atos oficiais do Pró-Gestão RPPS</p>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold text-white transition-opacity group-hover:opacity-90" style={{ backgroundColor: primaryColor }}>
                Acessar Canal <i className="ri-external-link-line"></i>
              </span>
            </a>
          ) : (
            <a
              ...
              target={isExternal ? "_blank" : undefined}
              rel={isExternal ? "noopener nofollow noreferrer" : undefined}
              className="flex-shrink-0 flex flex-col items-center justify-center p-6 bg-[#00266a] rounded-2xl hover:shadow-lg transition-all duration-300 cursor-pointer group text-center w-full md:w-72 border border-[#003580]"
            >
              <div className="relative w-28 h-28 flex items-center justify-center mb-4">
                <div className="absolute inset-0 rounded-full border-2 border-yellow-400/20 animate-pulse"></div>
                <div className="w-20 h-20 rounded-full border-2 border-yellow-400/40 flex flex-col items-center justify-center text-center p-2 bg-[#001f4d]">
                  <i className="ri-medal-2-line text-2xl text-yellow-400 mb-0.5"></i>
                  <span className="text-white font-extrabold text-[8px] leading-none">PRÓ-GESTÃO</span>
                </div>
              </div>
              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-1">Certificação Pró-Gestão</h4>
              <p className="text-[10px] text-white/60 leading-snug max-w-[200px] mb-3">Acesse a pasta de publicações e atos oficiais do Pró-Gestão RPPS</p>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold text-gray-900 bg-yellow-400 transition-colors group-hover:bg-yellow-300">
                Acessar Canal <i className="ri-external-link-line"></i>
              </span>
            </a>
          )}
