...
// GET /api/eventos/admin  (admin)
router.get("/admin", autenticar, exigirPermissao("eventos-inscritos"), async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const eventos = await query(
      `SELECT id, titulo, tipo, status, data_inicio, hora_inicio, local, online,
              link_online, palestrante, vagas, vagas_restantes, imagem_url,
              certificado, carga_horaria, categoria, publicado, destaque
       FROM eventos
       ORDER BY data_inicio DESC, id DESC`
    );
    res.json({ sucesso: true, dados: eventos.rows });
  } catch {
    res.status(500).json({ sucesso: false, mensagem: "Erro ao listar eventos no painel." });
  }
});

// GET /api/eventos/:id/inscricoes  (admin)
router.get("/:id/inscricoes", autenticar, exigirPermissao("eventos-inscritos"), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const inscritos = await query(
      `SELECT i.*, e.titulo AS evento_titulo, e.data_inicio
       FROM eventos_inscritos i
       JOIN eventos e ON e.id = i.evento_id
       WHERE i.evento_id = $1
       ORDER BY i.criado_em DESC, i.id DESC`,
      [req.params.id]
    );
    res.json({ sucesso: true, dados: inscritos.rows });
  } catch {
    res.status(500).json({ sucesso: false, mensagem: "Erro ao listar inscritos do evento." });
  }
});

// GET /api/eventos/inscricoes/admin  (admin)
router.get("/inscricoes/admin", autenticar, exigirPermissao("eventos-inscritos"), async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const inscritos = await query(
      `SELECT i.*, e.titulo AS evento_titulo, e.data_inicio
       FROM eventos_inscritos i
    ...
    res.status(500).json({ sucesso: false, mensagem: "Erro ao listar inscritos no painel." });
  }
});

// POST /api/eventos/inscricoes/admin  (admin)
router.post(
  "/inscricoes/admin",
  autenticar,
  exigirPermissao("eventos-inscritos"),
  auditoria("criar", "eventos-inscritos"),
  [
    body("eventoId").notEmpty().withMessage("Evento obrigatório"),
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const { eventoId, nome, email, cpf, matricula, cargo, telefone, status, presenca } = req.body;
    const statusDb = status === "pendente" ? "lista-espera" : (status ?? "confirmado");

    try {
      const inscrito = await queryOne(
        `INSERT INTO eventos_inscritos (evento_id, nome, email, cpf, matricula, cargo, telefone, status, presenca)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
        [eventoId, nome, email, cpf ?? null, matricula ?? null, cargo ?? null, telefone ?? null, statusDb, presenca ?? 0]
      );
      ...
      if (msg.toLowerCase().includes("duplicate")) {
        ...
        res.status(500).json({ sucesso: false, mensagem: "Erro ao criar inscrição." });
      }
    }
  }
);

// PUT /api/eventos/inscricoes/:inscricaoId  (admin)
router.put(
  "/inscricoes/:inscricaoId",
  autenticar,
  exigirPermissao("eventos-inscritos"),
  auditoria("editar", "eventos-inscritos"),
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const mapeados: Record<string, unknown> = {
      ...
      certificado_emitido: campos.certificado_emitido,
      status: campos.status === "pendente" ? "lista-espera" : campos.status,
    };
    ...
    if (sets.length === 0) {
      res.status(400).json({ sucesso: false, mensagem: "Nenhum campo para atualizar." });
      return;
    }

    vals.push(req.params.inscricaoId);

    try {
      const atualizado = await queryOne(`UPDATE eventos_inscritos SET ${sets.join(", ")} WHERE id = $${vals.length} RETURNING *`, vals);
      if (!atualizado) {
        res.status(404).json({ sucesso: false, mensagem: "Inscrição não encontrada." });
        return;
      }
      res.json({ sucesso: true, dados: atualizado });
    } catch {
      res.status(500).json({ sucesso: false, mensagem: "Erro ao atualizar inscrição." });
    }
  }
);

// DELETE /api/eventos/inscricoes/:inscricaoId  (admin)
router.delete(
  "/inscricoes/:inscricaoId",
  autenticar,
  exigirPermissao("eventos-inscritos"),
  auditoria("excluir", "eventos-inscritos"),
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      await query("DELETE FROM eventos_inscritos WHERE id = $1", [req.params.inscricaoId]);
      res.json({ sucesso: true, mensagem: "Inscrição removida." });
    } catch {
      res.status(500).json({ sucesso: false, mensagem: "Erro ao remover inscrição." });
    }
  }
);
