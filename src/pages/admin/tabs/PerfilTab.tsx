...
export default function PerfilTab() {
  ...
  const { usuarioLogado, salvarUsuarios, usuarios } = useAdminAuth();
  ...
  const handleSalvarDados = () => {
    ...
    setErroDados("");
    const atualizado = { ...usuarioLogado!, nome: nome.trim(), descricao, avatar };
    const novaLista = usuarios.map(u => u.id === atualizado.id ? atualizado : u);
  ...
  const handleTrocarSenha = () => {
    ...
    if (senhaAtual !== usuarioLogado?.senha) { setErroSenha("Senha atual incorreta."); return; }
