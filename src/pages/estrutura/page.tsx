...
function GestorModal({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl w-full max-w-4xl overflow-hidden animate-fade-in flex flex-col max-h-[96vh]">
        {/* Header com foto */}
        <div
...
export default function EstruturalPage() {
  ...
  const [lista, setLista] = useState<Gestor[]>(() => getGestores());
  const { config } = useSiteConfig();
  const { ref: heroRef, isVisible: heroVisible } = useScrollAnimation();
  const { ref: contentRef, isVisible: contentVisible } = useScrollAnimation();

  useEffect(() => {
    const atualizar = () => setLista(getGestores());
  ...
  const gruposFiltro = grupos.map(g => ({
    ...g,
    membros: lista.filter(gs => gs.grupo === g.key),
  }));
