...
function GestorModal({ gestor, onClose, primaryColor }: { gestor: Gestor; onClose: () => void; primaryColor: string }) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[96vh] overflow-hidden flex flex-col">
        {/* Header com foto */}
        <div
...
function carregarGestoresAtualizados(): Gestor[] {
  try {
    const saved = localStorage.getItem("inprec_gestores");
...
export default function GestoresPage() {
  ...
  const [listaGestores, setListaGestores] = useState<Gestor[]>(() => {
    return carregarGestoresAtualizados();
  });
  const { ref: heroRef, isVisible: heroVisible } = useScrollAnimation({ threshold: 0.1 });
  const { ref: contentRef, isVisible: contentVisible } = useScrollAnimation({ threshold: 0.05 });

  const gestoresFiltrados = listaGestores.filter(g => g.grupo === grupoAtivo);
  const grupoInfo = grupos.find(g => g.key === grupoAtivo);

  useEffect(() => {
    const atualizar = () => setListaGestores(carregarGestoresAtualizados());
    window.addEventListener("storage", atualizar);
    window.addEventListener("inprec-gestores-updated", atualizar);
    return () => {
      window.removeEventListener("storage", atualizar);
      window.removeEventListener("inprec-gestores-updated", atualizar);
    };
  }, []);
            ...
            {grupos.map((g) => (
              <div key={g.key} className="text-center">
                <p className="text-xl md:text-2xl font-bold text-white" style={{ fontFamily: "'Poppins', sans-serif" }}>
                  {listaGestores.filter(gs => gs.grupo === g.key).length}
                </p>
