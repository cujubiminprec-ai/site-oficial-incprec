...
export default function EleicaoBanner() {
  ...
  const [eleicao, setEleicao] = useState<EleicaoData>(() => {
    try {
      const saved = localStorage.getItem("inprec_eleicao_config");
      const votConfig = localStorage.getItem("inprec_votacao_config");
      const parsed = saved ? JSON.parse(saved) : defaultEleicao;
      const votParsed = votConfig ? JSON.parse(votConfig) : {};
      return { ...defaultEleicao, ...parsed, ativo: votParsed.ativa !== undefined ? votParsed.ativa : true };
    } catch {
      return defaultEleicao;
    }
  });
  ...
  useEffect(() => {
    const onStorage = () => {
      try {
        ...
        const parsed = saved ? JSON.parse(saved) : defaultEleicao;
        const votParsed = votConfig ? JSON.parse(votConfig) : {};
        setEleicao({ ...defaultEleicao, ...parsed, ativo: votParsed.ativa !== undefined ? votParsed.ativa : true });
      } catch { /* ignore */ }
    };
