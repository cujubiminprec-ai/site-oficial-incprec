...
function loadSlides(): SlideDisplay[] {
  try {
...
export default function HeroSlider() {
  const [slides, setSlides] = useState<SlideDisplay[]>(() => loadSlides());
  const [current, setCurrent] = useState(0);
  const [progress, setProgress] = useState(0);
  const INTERVAL = 5000;

  // Re-carrega slides quando localStorage muda (admin salvou)
  useEffect(() => {
    let alive = true;
    conteudoService.listarSlidesPublicos()
      .then((remote) => {
      ...
      .catch(() => {
        // Mantem fallback local para a home nao quebrar se a API estiver offline.
      });
    const handleSlidesUpdate = () => {
      const next = loadSlides();
      setSlides(next);
      setCurrent(0);
      setProgress(0);
    };
    window.addEventListener("storage", handleSlidesUpdate);
    window.addEventListener("inprec-slides-updated", handleSlidesUpdate);
    ...
    return () => {
      ...
      window.removeEventListener("inprec-slides-updated", handleSlidesUpdate);
