...
const NOTICIAS_KEY = "inprec_noticias_admin";
...
function getNoticiasPublicadasLocal(): HomeNoticia[] {
  try {
    const saved = localStorage.getItem(NOTICIAS_KEY);
    const lista: HomeNoticia[] = saved ? JSON.parse(saved).map(normalizarNoticia) : [];
    const ids = new Set(lista.map((n) => String(n.id)));

    noticiasMock.forEach((n) => {
      if (!ids.has(String(n.id))) {
        lista.push(normalizarNoticia({ ...n, publicada: true }));
      }
    });

    return ordenarNoticiasRecentes(lista.filter((n) => n.publicada !== false));
  } catch {
    return ordenarNoticiasRecentes(noticiasMock.map((n) => normalizarNoticia({ ...n, publicada: true })));
  }
}

export default function NoticiasSection() {
  const { config } = useSiteConfig();
  const [noticias, setNoticias] = useState<HomeNoticia[]>(getNoticiasPublicadasLocal);

  useEffect(() => {
    let ativo = true;
    const carregarLocal = () => {
      if (ativo) setNoticias(getNoticiasPublicadasLocal());
    };
    ...
    noticiasService
      ...
      .then((apiLista: ApiNoticia[]) => {
        if (!ativo) return;
        const local = getNoticiasPublicadasLocal();
        const apiNormalizada = apiLista.map(normalizarNoticia);
        
        // Mesclar as duas listas, evitando duplicatas por ID
        const finalMap = new Map<string | number, HomeNoticia>();
        local.forEach(n => finalMap.set(String(n.id), n));
        apiNormalizada.forEach(n => finalMap.set(String(n.id), n));
        
        const finalLista = ordenarNoticiasRecentes(Array.from(finalMap.values()));
        setNoticias(finalLista.filter(n => n.publicada !== false));
      })
      .catch(carregarLocal);

    window.addEventListener("storage", carregarLocal);
    window.addEventListener("inprec-noticias-updated", carregarLocal);
    return () => {
      ativo = false;
      window.removeEventListener("storage", carregarLocal);
      window.removeEventListener("inprec-noticias-updated", carregarLocal);
    };
  }, []);
