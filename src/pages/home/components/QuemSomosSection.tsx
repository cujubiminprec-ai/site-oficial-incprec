...
import { grupos, Gestor, Curso, Documento } from "@/mocks/gestores";
import { GESTORES_UPDATED_EVENT, carregarGestoresAtualizados } from "@/utils/gestoresData";
...
function PerfilGestorModal({ gestor, onClose, primaryColor }: { gestor: Gestor; onClose: () => void; primaryColor: string }) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[94vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="overflow-y-auto flex-1 px-6 md:px-10 py-5">
          {abaAtiva === "dados" && (
...
export default function QuemSomosSection() {
  ...
  const [listaGestores, setListaGestores] = useState<Gestor[]>(() => carregarGestoresAtualizados());

  useEffect(() => {
    const atualizar = () => setListaGestores(carregarGestoresAtualizados());
    window.addEventListener("storage", atualizar);
    window.addEventListener(GESTORES_UPDATED_EVENT, atualizar);
    return () => {
      window.removeEventListener("storage", atualizar);
      window.removeEventListener(GESTORES_UPDATED_EVENT, atualizar);
    };
  }, []);

  const diretoria = listaGestores.filter((gestor) => gestor.grupo === "diretoria" && gestor.ativo).slice(0, 5);
