...
import { DocumentoTransparencia, STORAGE_KEY_TRANSPARENCIA, documentos, menusLGPD } from "@/mocks/transparencia-docs";
...
function useDocsLGPD() {
  return useMemo(() => {
    try {
      ...
      return (source as Partial<DocumentoTransparencia>[])
        .map(normalizeDoc)
        .filter((d) => d.secao === "lgpd" && d.ativo !== false && !!d.link);
    } catch {
      return documentos.map(normalizeDoc).filter((d) => d.secao === "lgpd" && d.ativo !== false && !!d.link);
    }
  }, []);
}
