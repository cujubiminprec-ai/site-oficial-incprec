...
function getTodasNoticiasLocal(): PublicNoticia[] {
  try {
    ...
    const lista = s ? JSON.parse(s) : noticiasMock.map((n: typeof noticiasMock[0]) => ({ ...n, publicada: true, images: n.images || [] }));
    const ids = new Set(lista.map((n: { id: number }) => n.id));
    noticiasMock.forEach((n: typeof noticiasMock[0]) => {
...
export default function NoticiaDetalhePage() {
  ...
  useEffect(() => {
    ...
    noticiasService
      .listar()
      .then((lista) => {
        if (ativo && lista.length > 0) setTodasNoticias(ordenarNoticiasRecentes(lista.map(normalizarNoticiaApi)));
      })
      .catch(() => {
