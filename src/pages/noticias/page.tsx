...
import { todasNoticias as noticiasMock, categorias as categoriasMock } from "@/mocks/noticias-extra";
...
function getNoticiasPublicadasLocal(): PublicNoticia[] {
  try {
    const s = localStorage.getItem(NOTICIAS_KEY);
    const lista = s ? JSON.parse(s) : noticiasMock.map((n) => ({ ...n, publicada: true, images: n.images || [] }));
    const ids = new Set(lista.map((n: { id: number }) => n.id));
    noticiasMock.forEach((n) => {
      if (!ids.has(n.id)) lista.unshift({ ...n, publicada: true, images: n.images || [] });
    });
    return lista.filter((n: { publicada?: boolean }) => n.publicada !== false);
  } catch {
    return noticiasMock;
  }
}
  ...
  useEffect(() => {
    ...
    noticiasService
      ...
      .then((apiLista) => {
        ...
        const finalMap = new Map<string | number, PublicNoticia>();
