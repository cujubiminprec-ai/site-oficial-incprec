...
export default function ConsultaProtocolo() {
  ...
  const buscar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResultado(response.encontrado ? response.manifestacao ?? null : null);
    } catch {
      setResultado(null);
    } finally {
      setBuscado(true);
      ...
      <div className="p-6 flex flex-col gap-4">
        ...
        <div className="bg-gray-50 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-xs text-gray-500">{formatarData(resultado.data_resposta || "")}</p>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-1.5">
            <div
              ...
              style={{
                ...
                width: resultado.status === "respondida"
                  ? "100%"
        ...
        <div>
          <p className="text-xs font-semibold text-gray-500 mb-3">Histórico</p>
          <div className="flex flex-col gap-2">
            {[
              { status: "pendente", label: "Manifestação recebida", data: resultado.created_at },
              resultado.status === "em_analise" || resultado.status === "respondida"
                ? { status: "em_analise", label: "Em análise pela equipe", data: null }
                : null,
              resultado.status === "respondida"
                ? { status: "respondida", label: "Respondida", data: resultado.data_resposta }
                : null,
            ]
              .filter(Boolean)
              .map((etapa) => {
                const cfg = STATUS_CONFIG[etapa!.status as keyof typeof STATUS_CONFIG];
                return (
                  <div key={etapa!.status} className="flex items-center gap-3">
                    ...
                    >
                      <i className={cfg.icone}></i>
                    </div>
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-gray-700">{etapa!.label}</p>
                      {etapa!.data && (
