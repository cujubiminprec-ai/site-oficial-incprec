...
export default function ImageGalleryUploader({ images, onChange, primaryColor, maxImages = 10, uploadFolder = "noticias" }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      const uploads = await uploadService.uploadMultiplo(limited, uploadFolder);
      const newImgs: GalleryImage[] = uploads.map((uploaded, i) => ({
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <p className="text-xs font-semibold text-gray-700">
          {uploading ? "Enviando imagens..." : "Clique para adicionar imagens do computador"}
        </p>
        <p className="text-[11px] text-gray-400">PNG, JPG, WEBP · Salva em public/uploads/{uploadFolder} · Máx. {maxImages} imagens</p>
      </div>

      {uploadError && <p className="text-[11px] text-red-500 flex items-center gap-1"><i className="ri-error-warning-line"></i>{uploadError}</p>}
