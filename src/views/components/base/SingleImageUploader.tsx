...
export default function SingleImageUploader({
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadError("");
    try {
      const uploaded = await uploadService.upload(file, uploadFolder);
        ...
        <div className={`flex flex-col gap-2 ${previewShape === "wide" ? "w-full" : "flex-1 min-w-0"}`}>
          ...
          {showUrlInput && (
            <div>
              ...
              {uploadError && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><i className="ri-error-warning-line"></i>{uploadError}</p>}
              {uploadOk && <p className="text-[10px] mt-1 flex items-center gap-1" style={{ color: primaryColor }}><i className="ri-checkbox-circle-line"></i>Imagem salva em public/uploads/{uploadFolder}</p>}
            </div>
          )}
