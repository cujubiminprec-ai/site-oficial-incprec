...
export function normalizeHomeSections(list: Partial<HomeSectionConfig>[] | null | undefined): HomeSectionConfig[] {
  ...
  const custom = saved
    .filter((item) => (item.kind === "custom" || item.kind === "shortcuts") && item.id && !homeSectionsDefault.some((def) => def.id === item.id))
    .map((item, index) => ({
      id: item.id as HomeSectionKey,
      kind: item.kind === "shortcuts" ? "shortcuts" as const : "custom" as const,
      titulo: item.titulo || (item.kind === "shortcuts" ? "Atalhos importantes" : `Bloco personalizado ${index + 1}`),
      ...
      atalhos: Array.isArray(item.atalhos) ? item.atalhos.map((atalho, atalhoIndex) => ({
        id: atalho.id || `atalho-${atalhoIndex + 1}`,
        titulo: atalho.titulo || `Atalho ${atalhoIndex + 1}`,
        descricao: atalho.descricao || "",
        icone: atalho.icone || "ri-arrow-right-line",
        cor: atalho.cor || "#059669",
        linkUrl: atalho.linkUrl || "/",
        tipoDestino: atalho.tipoDestino || "link",
        arquivoNome: atalho.arquivoNome || "",
        arquivoTipo: atalho.arquivoTipo || "",
      })) : [],
    }));
...
export function saveHomeSections(sections: Partial<HomeSectionConfig>[]): HomeSectionConfig[] {
  return normalizeHomeSections(sections);
}
