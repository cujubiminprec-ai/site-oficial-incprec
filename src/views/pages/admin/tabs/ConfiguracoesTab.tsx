...
export default function ConfiguracoesTab() {
  ...
  const [form, setForm] = useState<SiteConfig & {
    ...
    contrachequeUrl: config.contrachequeUrl || "",
    horarioSeg: config.horarioSeg || "Segunda a Sexta: 07h30 às 13h30",
    horarioSab: config.horarioSab || "",
    horarioDom: config.horarioDom || "",
    redeFacebook: config.redeFacebook || "https://facebook.com/inprec",
    redeInstagram: config.redeInstagram || "https://instagram.com/inprec",
    redeYoutube: config.redeYoutube || "https://youtube.com/@inprec",
    redeLinkedin: config.redeLinkedin || "",
    mapEmbedUrl: config.mapEmbedUrl || "https://www.google.com/maps/embed",
    ...
    proGestaoLocais: config.proGestaoLocais || "navbar,footer,home,conteudo",
  });
  ...
  const proGestaoLocais = (form.proGestaoLocais || "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);

  const toggleProGestaoLocal = (local: string) => {
    const next = proGestaoLocais.includes(local)
      ? proGestaoLocais.filter((item) => item !== local)
      : [...proGestaoLocais, local];
    upd("proGestaoLocais", next.join(","));
  };
  ...
  const proGestaoLocalOptions = [
    {
      key: "navbar",
      label: "Menu superior",
      desc: "Mostra o selo ao lado da identidade INPREC, perto do início do site.",
      icon: "ri-layout-top-line",
    },
    {
      key: "home",
      label: "Seção da Home",
      desc: "Usa o selo real na área Pró-Gestão da página inicial.",
      icon: "ri-home-4-line",
    },
    {
      key: "conteudo",
      label: "Páginas de conteúdo",
      desc: "Exibe o card da certificação em páginas institucionais editáveis.",
      icon: "ri-pages-line",
    },
    {
      key: "footer",
      label: "Rodapé",
      desc: "Mostra o certificado na faixa de selos no final do site.",
      ...
      <div className="flex flex-col gap-6 max-w-2xl">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Controle a imagem, o link e onde a certificação aparece no site.
          </p>
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4 p-4 rounded-xl bg-gray-50">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5 mb-2">
                    Esta imagem será usada nos locais marcados abaixo.
                  </p>
                  <button
                    type="button"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-2 block">Onde exibir a certificação</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {proGestaoLocalOptions.map((item) => {
                  const ativo = proGestaoLocais.includes(item.key);
                  return (
                    <button
                      key={item.key}
                      type="button"
                      onClick={() => toggleProGestaoLocal(item.key)}
                      className="flex items-start gap-3 rounded-xl border p-3 text-left transition-all cursor-pointer"
                      style={ativo ? { borderColor: "#D97706", backgroundColor: "#D977060d" } : { borderColor: "#E5E7EB", backgroundColor: "#fff" }}
                    >
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: ativo ? "#D9770618" : "#F3F4F6" }}>
                        <i className={`${item.icon} text-base`} style={{ color: ativo ? "#D97706" : "#9CA3AF" }}></i>
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                          {item.label}
                          {ativo && <i className="ri-checkbox-circle-fill text-amber-600"></i>}
                        </p>
            ...
            >
              ...
              <div>
                <span className="text-sm font-semibold text-gray-900">
                  {form.proGestaoVisivel ? "Certificação ativa no site" : "Certificação desativada"}
                </span>
                <p className="text-xs text-gray-400">
                  {form.proGestaoVisivel ? "O selo aparece nos locais selecionados acima" : "O selo fica oculto em todos os locais"}
                </p>
              </div>
            </div>
          </div>
