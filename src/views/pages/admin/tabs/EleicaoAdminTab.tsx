...
export default function EleicaoAdminTab() {
  ...
  const [form, setForm] = useState<EleicaoConfig>(defaultConfig);
  ...
  useEffect(() => {
    ...
    eleicaoService.obterConfig()
      .then((dados) => {
        if (ativo) setForm({ ...defaultConfig, ...dados });
      })
      .catch(() => {
        if (ativo) setForm(defaultConfig);
      })
      .finally(() => {
  ...
  const handleSave = async () => {
    const salvo = await eleicaoService.salvarConfig(form);
    setForm({ ...defaultConfig, ...salvo });
    window.dispatchEvent(new Event("inprec-eleicao-updated"));
    setSaved(true);
