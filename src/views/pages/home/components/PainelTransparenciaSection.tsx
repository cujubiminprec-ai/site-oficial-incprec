...
export default function PainelTransparenciaSection() {
  ...
  const fetchPainel = useCallback(async () => {
    try {
      const [data, savedConfig] = await Promise.all([
        transparenciaService.listarPainel(),
      ...
      setPainelConfig(
        savedConfig
          ? { ...painelTransparenciaConfigDefault, ...savedConfig }
  ...
  useEffect(() => {
    ...
    window.addEventListener("inprec-painel-transparencia-updated", handleRefresh);
    return () => {
      window.removeEventListener("inprec-painel-transparencia-updated", handleRefresh);
    };
  }, [fetchPainel]);
