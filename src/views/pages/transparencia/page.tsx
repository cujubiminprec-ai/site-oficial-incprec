...
function useDocsTransparencia() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      transparenciaService.listarFinancas(),
    ]).then(([documentosResp, financasResp]) => {
      if (!ativo) return;
      const docsPublicos = documentosResp.status === "fulfilled"
        ? documentosResp.value.map((doc) => normalizeDoc(doc as Partial<DocumentoTransparencia>))
        : documentos.map(normalizeDoc);
      const docsFin = financasResp.status === "fulfilled" ? financasResp.value.map(normalizeDocFinanceiro) : docsFinanceiros.map(normalizeDocFinanceiro);
      setDocs(dedupeDocs([...docsPublicos, ...docsFin]).filter((d) => d.ativo !== false && !!d.link));
    });
...
function MenuPrimarioSection() {
  ...
  const { config } = useSiteConfig();
  const [menuPrimarioAtivos, setMenuPrimarioAtivos] = useState<TransparenciaMenu[]>(obterMenusPrimariosDinamicos().filter((m) => m.ativo !== false));
  ...
  useEffect(() => {
    ...
    transparenciaService
      .listarMenus()
      .then((menus) => {
        if (ativo && menus.length > 0) setMenuPrimarioAtivos(menus.filter((m) => m.ativo !== false));
      })
      .catch(() => undefined);
    return () => {
      ativo = false;
