...
export default function VotacaoPage() {
  ...
  const [candidatos, setCandidatos] = useState<Candidato[]>(candidatosDefault);
  const [votacaoConfig, setVotacaoConfig] = useState<VotacaoConfig>(votacaoConfigDefault);
  const [eleicaoConfig, setEleicaoConfig] = useState({ titulo: "Eleicao INPREC 2026", status: "em_andamento" });
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      try {
        const [lista, votacao, eleicao] = await Promise.all([
          eleicaoService.listarCandidatos().catch(() => candidatosDefault),
          eleicaoService.obterVotacaoConfig().catch(() => votacaoConfigDefault),
          eleicaoService.obterConfig().catch(() => ({ titulo: "Eleicao INPREC 2026", status: "em_andamento" })),
        ]);
        if (!ativo) return;
        setCandidatos(lista);
        setVotacaoConfig({ ...votacaoConfigDefault, ...votacao });
        setEleicaoConfig(eleicao);
      } catch {
        if (!ativo) return;
        setCandidatos(candidatosDefault);
        setVotacaoConfig(votacaoConfigDefault);
      }
    };
    carregar();
